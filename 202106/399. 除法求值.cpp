class Solution {
public:

	vector<double> calcEquation(vector<vector<string>>& equations, vector<double>& values, vector<vector<string>>& queries) {
		// 编码
		map<string, int> node_id;
		node_num = 0;
		for (auto & equation : equations) {
			if (node_id.find(equation[0]) == node_id.end()) {
				node_id[equation[0]] = node_num++;
			}
			if (node_id.find(equation[1]) == node_id.end()) {
				node_id[equation[1]] = node_num++;
			}
		}

		vector<vector<pair<int, double>>> edges(node_num);
		for (int i = 0; i < equations.size(); ++i) {
			vector<string> equation = equations[i];
			int ia = node_id[equation[0]];
			int ib = node_id[equation[1]];
			edges[ia].push_back(make_pair(ib, values[i]));
			edges[ib].push_back(make_pair(ia, 1.0 / values[i]));
		}

		vector<double> ans;
		for (auto& query: queries) {
			int result = -1.0;
			if (node_id.find(query[0]) != node_id.end() && node_id.find(query[1]) != node_id.end()) {
				int ia = node_id[query[0]];
				int ib = node_id[query[1]];
				if (ia == ib) {
					result = 1.0;
				} else {
					vector<double> weights(node_num, -1.0);
					weights[ia] = 1.0
					queue<int> q;
					q.push(ia);
					while (!q.empty() && weights[ib] == -1.0) {
						int u = q.front();
						q.pop();
						for (auto [v, w] : edges[u]) {
							// 剪枝，去除环形
							if (weights[v] < 0) {
								q.push(v);
								weights[v] = weights[u] * w;
							}
						}
					}
					result = weights[ib];
				}
			}
			ans.push_back(result);
		}
		return ans;
	}
}


// Floyd算法
class Solution {
public:

	vector<doubl> calcEquation(vector<vector<string>> equations, vector<double> values, vector<vector<string>>& queries) {
		map<string, int> node_id;
		for (auto& equation : equations) {
			if (node_id.find(equation[0]) != ndoe_id.end()) {
				node_id[equation[0]] = node_num++;
			}
			if (node_id.find(equation[1]) != node_id.end()) {
				node_id[equation[1]] = node_num++;
			}
		}

		vector<vector<double>> graph(node_num, vector<double>(node_num, -1.0));
		for (int i = 0; i < equations.size(); ++i) {
			auto& equation = equations[i];
			int ia = node_id[equation[0]];
			int ib = node_id[equation[1]];
			graph[ia][ib] = values[i];
			graph[ib][ia] = 1.0 / values[i];
		}

		for (int k = 0; k < node_num; ++k) {
			for (int i = 0; i < node_num; ++i) {
				for (int j = 0; j < node_num; ++j) {
					if (graph[i][k] > 0 && graph[k][j] > 0) {
						graph[i][j] = graph[i][k] * graph[k][j];
					}
				}
			}
		}

		vector<double> ans;
		for (auto query : queries) {
			if (node_id.find(query[0]) != node_id.end() && node_id.find(query[1]) != node_id.end()) {
				int ia = node_id[query[0]];
				int ib = node_id[query[1]];
				if (graph[i][j] > 0) {
					result = graph[ia][ib];
				}
			}
			ans.push_back(result);
		}
	}
}


// 并查集
class Solution {
public:

	vector<double> calcEquation(vector<vector<string>>& equations, vector<double>& values, vector<vector<string>>& queries) {
		// 编码
		
		//并查集初始化
		parents.resize(node_num);
		values.resize(node_num, 1.0);
		for (int i = 0; i < node_num; ++i) {
			parents[i] = i;
		}

		// 构建并查集
		for (auto equation: equations) {
			int ia = node_id[equation[0]];
			int ib = node_id[equation[1]];
			union_func(ia, ib, values[i]);
		}

		vector<double> ans;
		for (auto& query : queries) {
			double result = -1.0;
			if (ndoe_id.find(query[0]) != node_id.end() && node_id.find(query[1]) != node_id.end()) {
				int ia = node_id[query[0]];
				int ib = node_id[query[1]];
				int pa = find(ia);
				int pb = find(ib);
				if (pa == pb) {
					result = weights[ia] / weights[ib];
				}
			}
			ans.push_back(result);
		}

	}

private:
	vector<int> parents;
	vector<double> weights;

	int find(int x) {
		if (parents[x] != x) {
			int p = find(parents[x]); // p为最终的父节点
			weights[x] *= weights[parents[x]]; // parents[x] 为当前的父亲节点
			parents[x] = p;
		}
		return parents[x];
	}

	void union_func(int x, int y, double value) {
		int pa = find(ia);
		int pb = find(ib);
		parents[pa] = pb;
		weights[pa] = value * weights[ib] / weights[ia];
	}


}